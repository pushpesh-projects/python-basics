...
A compiled language is a programming language that is generally compiled and
not interpreted.It is one where the program, once compiled, is expressed in the
instructions of the target machine;
this machine code is undecipherable by humans. Types of compiled language – C,
C++, C#, CLEO, COBOL, etc.
...
machine, but instead,read and executed by some other program. Interpreted
language ranges – JavaScript, Perl, Python, BASIC, etc.

Java is kind of both. Firstly java compiled(some would prefer to say
"translated") to bytecode,which then either compiled, or interpreted by JIT.
"""


"""
Computers have their own language, too, called machine language.
Python is an interpreted language. If you want to program in Python,
you'll need the Python interpreter.

Due to historical reasons, languages designed to be utilized in the
interpretation manner are often called scripting languages, while the source
...
X64 CPUs are fast and powerful, but they require a lot of electricity. So, they
are usually used in laptops.
ARM CPUs are weak but low-power processors for smartphones and other mobile
devices.Apple has released M1/2 laptops based on ARM CPUs and they have strong
performance. Many poeple think the future is moving to ARM CPUs.
"""
...
The PSF's president is Guido von Rossum himself, and for this reason, these
Pythons are called canonical.They are also considered to be reference Pythons,
as any other implementation of the language should follow all standards
established by the PSF.

Guido van Rossum used the "C" programming language to implement the very first
version of his language and this decision is still in force. All Pythons
coming from the PSF are written in the "C" language.

It is important to remember that there may be smaller or bigger differences
between subsequent Python 3 releases
(e.g., Python 3.6 introduced ordered dictionary keys by default under the
CPython implementation) –the good news, though,
is that all the newer versions of Python 3 are backward compatible with the
previous versions of Python 3.

Backward compatible means that code written for one version of Python 3
should generally work correctly when run on a later version of Python 3.

Cython: Cython is intended to do – to automatically translate the Python code
(clean and clear, but not too swift)
...
as C and Java live in completely different worlds and don't share many
common ideas.
