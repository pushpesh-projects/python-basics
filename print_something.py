...
escaping nature - you have to double it,
...
print("\\")
"""
The print() function - using multiple arguments: a print() function invoked
with more than one argument outputs them all on one line;
...
e.g., the second argument is outputted
...
but by a special word (keyword) used to identify them.The end and sep
parameters can be used for formatting the output of the print() function.
The sep parameter specifies the separator between the outputted arguments
(e.g., print("H", "E", "L", "L", "O", sep="-"),whereas the end parameter
...
"""
Literals
Literals are notations for representing some fixed values in code.Python has
various types of literals - for example,a literal can be a number
(numeric literals, e.g., 123), or a string (string literals, e.g.,
"I am a literal.").There is one more, special literal that is used in Python:
the None literal.This literal is a so-called NoneType object, and it is used to
 represent the absence of a value.Boolean values are the two constant objects
 True and False used to represent truth values
