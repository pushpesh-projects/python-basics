...
the int() function takes one argument (e.g., a string: int(string)) and tries
to convert it into an integer;
the float() function takes one argument (e.g., a string: float(string)) and
tries to convert it into a float.
...
# In the below line we don't type cast integer to string because we are not
# concatenating the string to integer.
